...
export default function SchematicPage() { const [toast, context] = message.useMessage(); const [assets, setAssets] = useState<Asset[]>([]); const [batchOpen, setBatchOpen] = useState(false); const [loadingBatchId, setLoadingBatchId] = useState<string | null>(null); const [busy, setBusy] = useState(false); const [dropActive, setDropActive] = useState(false); const [progress, setProgress] = useState<{ parse: ImportProgress; generate: ImportProgress }>({ parse: { phase: 'parse', completed: 0, total: 0 }, generate: { phase: 'generate', completed: 0, total: 0 } }); const [reviewOpen, setReviewOpen] = useState(false); const [confirmingReview, setConfirmingReview] = useState(false); const [screen, setScreen] = useState<'upload' | 'arrange'>('upload'); const [uploadFromArrange, setUploadFromArrange] = useState(false); const [pages, setPages] = useState<Page[]>([]); const [activePage, setActivePage] = useState(1); const [selectedItem, setSelectedItem] = useState<string | null>(null); const [selectedGroupIds, setSelectedGroupIds] = useState<string[]>([])
  ...
  const upload = useCallback(async (droppedPath?: string) => { setBusy(true); setDropActive(false); setProgress({ parse: { phase: 'parse', completed: 0, total: 0 }, generate: { phase: 'generate', completed: 0, total: 0 } }); const streamedIds = new Set<string>(); const wasOpen = batchOpen; if (!wasOpen && !currentBatchId) setCurrentBatchId(crypto.randomUUID()); try { const imported = await importAssets(next => setProgress(current => ({ ...current, [next.phase]: next })), batch => { batch.forEach(asset => streamedIds.add(asset.id)); setAssets(current => { const existing = new Set(current.map(asset => asset.id)); return [...current, ...batch.filter(asset => !existing.has(asset.id))] }); setBatchOpen(true) }, droppedPath); if (imported.length && streamedIds.size === 0) { setAssets(current => [...current, ...imported]); setBatchOpen(true) } if (imported.length) toast.success(wasOpen ? schematicCopy.adding : schematicCopy.creating) } catch (e) { if (streamedIds.size) { setAssets(current => current.filter(asset => !streamedIds.has(asset.id))); setSelectedAssetIds(current => new Set([...current].filter(id => !streamedIds.has(id)))); if (!wasOpen) { setBatchOpen(false); setCurrentBatchId(null) } } toast.error(String(e)) } finally { setBusy(false) } }, [batchOpen, currentBatchId, toast])
  ...
  const combineSelectedGroups = () => {
    ...
    try {
      ...
      const nextPages = autoArrangePages(next, start, assets, layoutBounds, productConfigs)
      setPages(nextPages)
      setSelectedGroupIds([])
      setSelectedItem(null)
  ...
  const autoArrange = () => {
    try {
      const nextPages = autoArrangePages(pages, activePage, assets, layoutBounds, productConfigs)
      setPages(nextPages)
