...
export default function SchematicPage() { const [toast, context] = message.useMessage(); const [assets, setAssets] = useState<Asset[]>([]); const [batchOpen, setBatchOpen] = useState(false); const [loadingBatchId, setLoadingBatchId] = useState<string | null>(null); const [busy, setBusy] = useState(false); const [dropActive, setDropActive] = useState(false); const [progress, setProgress] = useState<{ parse: ImportProgress; generate: ImportProgress }>({ parse: { phase: 'parse', completed: 0, total: 0 }, generate: { phase: 'generate', completed: 0, total: 0 } }); const [reviewOpen, setReviewOpen] = useState(false); const [confirmingReview, setConfirmingReview] = useState(false); const [externalImportPath, setExternalImportPath] = useState<string | null>(null); const [screen, setScreen] = useState<'upload' | 'arrange'>('upload'); const [uploadFromArrange, setUploadFromArrange] = useState(false); const [pages, setPages] = useState<Page[]>([]); const [activePage, setActivePage] = useState(1); const [selectedItem, setSelectedItem] = useState<string | null>(null); const [selectedGroupIds, setSelectedGroupIds] = useState<string[]>([])
  ...
  const upload = useCallback(async (droppedPath?: string, forceNewBatch = false) => { if (forceNewBatch) { const pendingAssets = assets; grouping.reset(); setPages([]); setSelectedItem(null); setSelectedGroupIds([]); setAssets([]); setSelectedAssetIds(new Set()); setCurrentBatchId(null); setBatchOpen(false); setBatchMetadata(defaultBatchMetadata); if (pendingAssets.length && isTauri()) void invoke('discard_temp_assets', { assets: pendingAssets }).catch(() => {}) } setBusy(true); setDropActive(false); setProgress({ parse: { phase: 'parse', completed: 0, total: 0 }, generate: { phase: 'generate', completed: 0, total: 0 } }); const streamedIds = new Set<string>(); const wasOpen = forceNewBatch ? false : batchOpen; let initializedName = false; const initializeBatchName = (imported: Asset[]) => {
  ...
  useEffect(() => { if (!isTauri()) return; let cancelled = false; let unlisten = () => {}; void listen<{ path: string }>('external-svg-import', event => { if (cancelled) return; if (screen === 'upload') void upload(event.payload.path); else { setExternalImportPath(event.payload.path); void getCurrentWindow().setFocus().catch(() => {}) } }).then(stop => { if (cancelled) stop(); else unlisten = stop }).catch(() => {}); return () => { cancelled = true; unlisten() } }, [screen, upload])
  const externalImportDialog = <Modal open={Boolean(externalImportPath)} title="接受到新图片是否处理新批次" footer={<><Button onClick={() => setExternalImportPath(null)}>取消</Button><Button onClick={() => { const path = externalImportPath; if (!path) return; setExternalImportPath(null); setUploadFromArrange(true); setScreen('upload'); void upload(path) }}>追加当前批次</Button><Button type="primary" onClick={() => { const path = externalImportPath; if (!path) return; setExternalImportPath(null); setUploadFromArrange(true); setScreen('upload'); void upload(path, true) }}>新建批次</Button></>}><p>当前正在示意图排列页，请选择如何处理这张 SVG。</p></Modal>
