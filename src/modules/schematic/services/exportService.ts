...
export async function exportPdf(pages: Page[], assets: Asset[], outputDir: string, baseName: string, metadata: Partial<BatchMetadata> = {}, onProgress?: (progress: PdfExportProgress) => void) {
  ...
  try {
    ...
    await invoke('export_staged_pdf_bundle', { outputDir, baseName, ids })
  } finally {
    unlisten()
