...
export function readGroupAssetEditor(asset: Asset): GroupEditor {
  return {
    ...
    custom: { id: Number(asset.productId.slice(7)) || null, name: asset.productName ?? '', size: asset.attributes?.Size ?? '', printOption: asset.attributes?.['Print Option'] ?? '', finish: asset.attributes?.Finish ?? '', accessoryColor: asset.attributes?.['Accessories Color'] ?? '', accessoryColorImage: asset.attributeImages?.['Accessories Color'] ?? '', qt: Number(asset.attributes?.QT) || 1 },
  }
}
