...
export type Item = { id: string; assetId: string; x: number; y: number; w: number; h: number; rotation: number; mirrorX?: boolean; backSvg?: string; derivedFrom?: string; caption?: string; rulerUnit?: 'mm' | 'cm' | 'in'; rulerWidth?: boolean; rulerHeight?: boolean; suppressRuler?: boolean }
...
export function pageSvg(page: Page, assets: Map<string, Asset>, metadata: PageHeader = {}, totalPages = 1) {
  const images = page.items.map(item => {
    ...
    return `${item.caption ? `<text x="${item.x}" y="${item.y - item.h / 2 - 10}" text-anchor="middle" font-size="10" fill="#e11d48">${xmlText(item.caption)}</text>` : ''}<image x="${-item.w / 2}" y="${-item.h / 2}" width="${item.w}" height="${item.h}" transform="translate(${item.x} ${item.y}) rotate(${item.rotation}) scale(${item.mirrorX ? -1 : 1} 1)" href="${svgDataUrl(item.backSvg ?? asset.svg)}"/>`
  }).join('')
